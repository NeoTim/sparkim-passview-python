...
# This program is based on, Spark IM Password Decrypter
# Copyright 2012 Adam Caudill <adam@adamcaudill.com>
# https://github.com/adamcaudill/sparkim-passview

if __name__ == "__main__":
    import sys
    import base64

    KEY = "ugfpV1dMC5jyJtqwVAfTpHkxqJ0+E0ae"

    try:
        from Crypto.Cipher import DES3
    except ImportError:
        print >> sys.stderr, "PyCrypto is require by this utility!"
        sys.exit(-2)

    if len(sys.argv) < 2:
        print >> sys.stderr, "Usage: %s <spark.properties file(s)>" % sys.argv[0]
        sys.exit(-1)

    for i in range(1, len(sys.argv)):
        try:
            p = Properties()
            p.load(open(sys.argv[i]))
            data = p.getPropertyDict()
        except:
            pass
        ...
        if not user or not server:
            print >> sys.stderr, "%s doesn't contain user / server name!" % sys.argv[i]
            continue

        password = data.get("password", None)
        if not password:
            # find password key (hack which bypasses implementing PKCS #7 encoding)
            for key in data.keys():
                if key.find("password") > -1 and key != "passwordSaved":
                    password = data[key]

        if not password:
            print >> sys.stderr, "%s doesn't contain encrypted password!" % sys.argv[i]
            continue
        ...
        output = des3.decrypt(password)
        print "%s:%s:%s:%s" % (sys.argv[i], user, server, output)
